...
ESX.SavePlayer = function(xPlayer, cb)
	...
	table.insert(asyncTasks, function(cb2)
		MySQL.Async.execute('UPDATE users SET accounts = @accounts, job = @job, job_grade = @job_grade, `group` = @group, loadout = @loadout, position = @position, job2 = @job2, job2_grade = @job2_grade, inventory = @inventory WHERE identifier = @identifier', {
